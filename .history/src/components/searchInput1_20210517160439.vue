...
export default defineComponent({
  ...
  setup () {
    ...
    const selectDown = () => {
      now.value++;
      if (now.value == myData.value.length) now.value = -1;
      console.log(now.value)
      console.log(JSON.stringify(myData.value))
      value.value = myData.value[now.value];
    };

    const selectUp = () => {
      now.value--;
      if (now.value == -2) now.value = myData.value.length - 1;
      value.value = myData.value[now.value];
    }
