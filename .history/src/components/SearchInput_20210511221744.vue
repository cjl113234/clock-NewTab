...
export default defineComponent({
  ...
  setup () {
    ...
    const handleSearch = async val => {
      ...
      myValue.value = value.value
      const { data: res } = await ctx.axios.get('/api/5a1Fazu8AA54nxGko9WTAnF6hhy/su?wd=' + myValue.value + '&json=1&cb=');
      myData.value = res.replace(/[(;)]/g, "")
      myData.value = JSON.parse(myData.value)
      ...
      console.log(dataSource.value[1])
